...
<body>
  <nav class="navbar text-dark navbar-expand-lg bg-dark mr-3">
    <div class="container-fluid">
      <a class="navbar-brand" href="#">Books R Us / Admin Dashboard/ Users</a>
      </div>
    </div>
  </nav>
  <table class="table mt-5">
    ...
    <?php 
      foreach ($paginated_data as $book_data) {
        echo '<tr>';
        echo '<th scope="col">' . $book_data['email'] . '</th>';
        echo '<th scope="col"><img src= "' . $book_data['user_avatar'] . '"style="height:10vh;width:10vw;"></th>';
        echo '<th scope="col">' . $book_data['acct_type'] . '</th>';
...
<script>
  var category_to_be_edited = document.querySelectorAll('#edit');
    ...
    category.addEventListener('click',(e)=>{
        updated_category = prompt('Enter new category name');
